# -*- coding: utf-8 -*-
"""
Created on Thu Jul  1 10:15:21 2021
...
fdir=str(args.topo.replace(".graphml", ""))+"jsonFilesMoon"
...
def flowLeader(flow):
    ...
    for node in f:
        temp=nodelist.index(str(node))
        #print ( 'temp: ', len(nodesRank))
        rankList.append(nodesRank[temp-1])
        
    min_rankList = min(rankList)
    min_index = rankList.index(min_rankList)
    rank = flow[min_index]
...
print ('P4Flow_leaderList: ', leaderList)
print ('P4Flow_hopsList: ', (hopsList))
print ('P4Flow_delayList: ', (delayList_leaderToController))
print ('P4Flow_avg_delay: ', np.mean(delayList_leaderToController))
print ('P4Flow_avg_hops: ', np.mean(hopsList))
print ('P4Flow_exeTime: ', exeTime+leader_time )
