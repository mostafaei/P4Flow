# -*- coding: utf-8 -*-
"""
Created on Fri Jul  2 17:40:25 2021
...
fdir=str(args.topo.replace(".graphml", ""))+"commandsFilesFlowCover"
...
""" 
 In flowCover nodes are ranked based on their occurence in the flows
 therefore, we checked each node of each flow in the other flows to 
 see how many times they're repeated. We take the number of repetition
 as the rank of each node in each flow. Finally, we picked the node with
 the highest rank as the leader.
 
""" 
def flowCoverRank(flowSet):
    for f in flowSet:
        sublist=[]
        for flow in f:
            counter = 0
...
#We ranked the nodes based on their weight, the node with the highest occurence among all flows selected as the leader 
flowCoverRank(flowSet)
...
for k in flowSet:
    ...
    bestPaths.append(k)
    maxValue = max(listoflists[flowCounter])
    l_index = listoflists[flowCounter].index(maxValue) 
    leader = flowSet[flowCounter][l_index]
...
print ('flowCover_leaderList: ', leaderList)
print ('flowCover_hopsList: ', (hopsList))
print ('flowCover_delayList: ', (delayList_leaderToController))
print ('flowCover_avg_delay: ', np.mean(delayList_leaderToController))
print ('flowCover_avg_hops: ', np.mean(hopsList))
print ('flowCover_exeTime: ', exeTime+leader_time )
